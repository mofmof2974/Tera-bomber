...
function boardTemplate(state) {
  ...
  const count = question.count === 10 ? 10 : 5;
  const positions = Array.from({ length: 5 }, (_, i) => i);
  const images = (question.images || []).slice(0, count);
  return `<section class="board-screen ${state.status}">
    <header class="topbar"><div class="brand">TERA <strong>BOMBER</strong></div><div class="question-number">QUESTION <b>${escapeHtml(question.number || '01')}</b></div><div class="timer">${String(Math.floor(state.remaining / 60)).padStart(2, '0')}:${String(state.remaining % 60).padStart(2, '0')}</div></header>
    <div class="question-panel ${question.type === 'image' ? 'image-question' : ''}">${question.type === 'image' ? images.map((image, i) => `<div class="question-image"><img src="${escapeHtml(image)}" alt="出題画像 ${i + 1}">${state.accepted.includes(i) ? '<span class="check">✓</span>' : ''}</div>`).join('') : `<p>${escapeHtml(question.text)}</p>`}</div>
...
function adminTemplate(state) {
  ...
  return `<section class="admin-shell"><header class="admin-header"><div><span class="eyebrow">CULTURAL FESTIVAL / CONTROL ROOM</span><h1>Tera Bomber <em>司会者コンソール</em></h1></div><a class="view-link" href="/" target="_blank">表示画面を開く ↗</a></header>
    <div class="admin-grid"><div class="control-panel"><h2>ROUND SETUP</h2><label>出題する問題<select id="question-select">${questionOptions.map((question) => `<option value="${escapeHtml(question.id)}">${escapeHtml(question.source || question.id)}</option>`).join('')}</select></label><div class="split"><label>問題タイプ<select id="question-type"><option value="text" ${q.type === 'text' ? 'selected' : ''}>問題文型</option><option value="image" ${q.type === 'image' ? 'selected' : ''}>画像出題型</option></select></label><label>回答数<select id="answer-count"><option value="5" ${q.count !== 10 ? 'selected' : ''}>5問</option><option value="10" ${q.count === 10 ? 'selected' : ''}>10問</option></select></label></div><label>問題文<textarea id="question-text">${escapeHtml(q.text || '')}</textarea></label><label>制限時間（秒）<input id="duration" type="number" min="5" max="600" value="${state.duration}"></label><button class="primary" id="apply-question">問題をセットする</button></div>
    <div class="control-panel live"><div class="panel-title"><h2>LIVE CONTROL</h2><span class="live-dot">● ${state.status.toUpperCase()}</span></div><div class="big-time">${String(Math.floor(state.remaining / 60)).padStart(2, '0')}:${String(state.remaining % 60).padStart(2, '0')}</div><div class="control-actions"><button class="primary" id="start">${state.running ? 'タイマー進行中' : 'タイマーを開始'}</button><button id="reset">ラウンドをリセット</button></div><div class="judge"><label>現在の回答<input id="answer" value="${escapeHtml(state.answerText)}" placeholder="回答を入力"></label><div><button class="correct" id="correct">正解 ✓</button><button class="wrong" id="wrong">不正解 ×</button></div></div><div class="position-control"><span>爆弾位置</span>${Array.from({ length: q.count === 10 ? 10 : 5 }, (_, i) => `<button class="position ${i === state.bombPosition ? 'selected' : ''}" data-position="${i}">${i + 1}</button>`).join('')}</div></div></div><div class="admin-note">問題ファイルは <strong>問題用フォルダ</strong> に JSON / TXT / MD 形式で追加できます。JSON は <code>{"type":"text","count":5,"text":"問題文"}</code> を基本形にしてください。</div></section>`;
}
  ...
  if (id === 'apply-question') {
    const selected =
      availableQuestions.find(
        (question) =>
          question.id === document.querySelector('#question-select').value,
      ) || {};
    socket.emit('admin:setQuestion', {
      ...selected,
      id: selected.id || 'manual',
      type: document.querySelector('#question-type').value,
      count: document.querySelector('#answer-count').value,
...
document.addEventListener('change', (event) => {
  ...
  if (!question) return;
  document.querySelector('#question-type').value = question.type || 'text';
  document.querySelector('#answer-count').value =
    question.count === 10 ? '10' : '5';
